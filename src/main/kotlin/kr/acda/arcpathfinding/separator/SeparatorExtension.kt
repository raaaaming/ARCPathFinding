package kr.acda.arcpathfinding.separator
...
fun buildOrderByChunkSeparator(
    ...
    for (u in 0 until N) {
        val cx = floorDiv(g.nodeX(u), chunkSize); val cz = floorDiv(g.nodeZ(u), chunkSize)
...
fun buildOrderByChunkSeparatorFast(
    ...
    val W = maxCx - minCx + 1; val H = maxCz - minCz + 1; val cellCount = W * H
    ...
    for (u in 0 until N) {
        val gx = cxArr[u] - minCx; val gz = czArr[u] - minCz
        val id = idx(gx, gz)
        if (buckets[id] == null) buckets[id] = ArrayList()
        buckets[id]!!.add(u); occ[id]++
    }
    ...
    fun regionHasAny(gx0: Int, gx1: Int, gz0: Int, gz1: Int): Boolean {
        ...
        val A = ps[psIdx(gz0, gx0)]; val B = ps[psIdx(gz0, gx1 + 1)]
        val C = ps[psIdx(gz1 + 1, gx0)]; val D = ps[psIdx(gz1 + 1, gx1 + 1)]
        return (D - B - C + A) > 0
    }
    ...
    fun addCell(gx: Int, gz: Int, depth: Int) {
        ...
        for (u in b) { if (level[u] == -1) { level[u] = depth; order[op++] = u } }
    }
    ...
    fun rec(gx0: Int, gx1: Int, gz0: Int, gz1: Int, depth: Int) {
        ...
        val w = gx1 - gx0 + 1; val h = gz1 - gz0 + 1
        if (w <= 1 && h <= 1) { addCell(gx0, gz0, depth); return }
        val t = sepThickness
        if (w >= h) {
            val mid = (gx0 + gx1) ushr 1
            val a = max(gx0, mid - (t - 1) / 2); val b = min(gx1, a + t - 1)
            ...
            val a = max(gz0, mid - (t - 1) / 2); val b = min(gz1, a + t - 1)
